...
module.exports = class Tesla extends EventEmitter {
    ...
    async navigationRequest(vehicleId, address, locale) {
        return this.postVehicleCommand(vehicleId, 'command/share', {
        ...
        if (this._socket) {
            ...
            this._socket.on('message', data => {
                ...
                if (parsed.msg_type === 'control:hello') {
                    this.logger.debug('Streaming websocket connected');
                ...
                this.logger.debug('Streaming websocket closed');
    ...
    _onMessage(data, callback) {
        ...
        const response = {
            ...
            range: Math.round(this._parseValue(values[9], 0) * MILES_TO_KM),
            est_range: Math.round(this._parseValue(values[10], 0) * MILES_TO_KM),

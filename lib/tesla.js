...
module.exports = class Tesla extends EventEmitter {
    ...
    async validateTokens() {
        if (!this.tokens) {
            this.logger.info('validateTokens. no tokens. will login');
            return this.login();
        }
        if (this.tokensExpired(this.tokens)) {
            try {
                const tokens = await this.teslaAuth.refresh(this.tokens.auth.refresh_token);
                this.tokens = tokens;

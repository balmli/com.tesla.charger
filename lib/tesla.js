    ...
    async validateGrant() {
        ...
        if (this.grantExpired(this.grant)) {
            return this._tokenRefresh()
                ...
                .catch(err => {
                    self.logger.error('validateGrant. tokenRefresh err', err);
                    self.login();
                });
        } else {
            return this._checkTokenWithRequest()
                .then(() => {
                    return self.grant;
                })
                .catch(err => {
                    self.logger.error('validateGrant. checkTokenWithRequest err', err);
                    self.login();
                });
        }
    }
        ...
        return http.get({
            ...
            .then(function (result) {
                if (!result.response || result.response.statusCode !== 200) {
                    return Promise.reject('_checkTokenWithRequest');
                }
            })
    }
